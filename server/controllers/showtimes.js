    ...
    getMovieShowtime: function (req, res) {
        sql.query('SELECT moveek_id FROM movies WHERE movie_id = ? AND deleted_at IS NULL', req.params.movie_id, (err, result) => {
            if (err) {
                ...
                showTimeCrawler.crawlCineIdAndMovieIdFromMoveek(result[0].moveek_id, getDay())
                    .then(response => {
                        res.json(response);
                    })
    ...
    getMovieShowtimeByCine: function (req, res) {
        sql.query('SELECT moveek_id FROM movies WHERE movie_id = ? AND deleted_at IS NULL', req.params.movie_id, (err, result) => {
            if (err) {
                ...
                showTimeCrawler.crawlMovieShowtimeFromCine(result[0].moveek_id, req.params.cine_id, getDay())
                    .then(response => {
                        res.json(response);
                    })
                    .catch(err => { res.send({ error: err }) });
    ...
    getCineByMovie: function (req, res) {
        sql.query("SELECT moveek_id FROM movies WHERE movie_id = ? and deleted_at IS NULL", req.params.movie_id, (err, result) => {
            if (err) {
                ...
                showTimeCrawler.crawlCineFromMovie(result[0].moveek_id, getDay())
                    .then(response => {
                        res.json(response);
                    })
                    .catch(err => { res.send({ error: err }) });
